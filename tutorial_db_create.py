...
def replace_query(db, name, description, filename, template):
    ...
    for q in existing:
        db.session.delete(q)
    newq = Query(name=name, description=description,
...
def replace_tabdata_query(db, name, description, parameters, template):
    ...
    for q in existing:
        db.session.delete(q)
    newq = Tabdata_query(name=name, description=description,
...
with app.app_context():
    ...
    replace_query(
        db, name="Subject",
        description="Analysis of a subject; most-published journals, authors, universities, countries",
        filename="subject_query.json", template="subject_query.html")
    replace_query(
        db, name="Country",
        description="Analysis of a country; number of papers by subject, university, collaborating country",
        filename="country_query.json", template="country_query.html")
    replace_query(
        db, name="Hexbin",
        description="Hexbin-and-arcs display of papers in a dataset; keep sets small!",
    ...
    if user_datastore.find_user(nickname="admin") is None:
        admin = user_datastore.create_user(
            nickname="admin", email="vputz@nyx.net", password="marion")
